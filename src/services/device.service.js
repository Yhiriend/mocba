...
export class DeviceService {
    async registerDevice(device, key) {
        try {
    ...
    async getDeviceById(id) {
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                const device = useDeviceStore().getDevices.find((device) => device.id === id);
                if (device) {
                    resolve(device);
                }
                else {
    ...
    async updateDevice(deviceId, updatedDeviceData) {
        try {
